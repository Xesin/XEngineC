...
public:
	...
	multicast_delegate& operator -=(const delegate<RET(PARAMS...)>& another) {
		if (another.isNull()) return *this;
		auto i = invocationList.begin();
		for (typename delegate_base<RET(PARAMS...)>::InvocationElement * element : invocationList) {
			if (element->stub == another.invocation.stub || element->object == another.invocation.object) {
				invocationList.erase(i);
				return *this;
			}
			i++;
		}
